...
interface UpdateScoresModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: { rank: number; percentile: number; correctAnswers: number }) => void;
  initialRank: number;
...
export default function UpdateScoresModal({ 
  ...
  const [errors, setErrors] = useState<{ rank?: string; percentile?: string; correctAnswers?: string }>({});

  if (!isOpen) return null; // Prevent rendering if modal is closed

  const validateAndSubmit = () => {
    let newErrors: { rank?: string; percentile?: string; correctAnswers?: string } = {};
    ...
    onSave({ rank: Number(rank), percentile: Number(percentile), correctAnswers: Number(correctAnswers) });
        ...
        <div className="mt-4 space-y-4">
          ...
          <div>
            <div className="flex items-center">
              ...
              <span>Update your</span> <span className="font-bold ml-1">Correct Answers (out of 15)</span>
              <input 
                type="number"
                className="border ml-auto p-2 w-20 rounded-md"
            ...
            {errors.correctAnswers && <p className="text-red-500 text-sm">{errors.correctAnswers}</p>}
          </div>
        </div>

        {/* Buttons */}
        <div className="flex justify-end gap-4 mt-4">
          <button className="border border-blue-600 text-blue-600 px-4 py-2 rounded-lg" onClick={onClose}>
            Cancel
          </button>
          <button className="bg-blue-600 text-white px-4 py-2 rounded-lg shadow-md hover:bg-blue-700" onClick={validateAndSubmit}>
            Save →
          </button>
        </div>
      </div>
